...
퀄리티가 높은 사진은 프로젝트의 질을 한층 더 높여줍니다. 하지만 과도한 사진 사용은 포트폴리오의 건강(?)에 상당한 해를 끼칠 수 있기 때문에 필자는 사진 사용을 꺼려합니다. 그리고 사진 쓰면 랜딩 시간이 길어져... 그래서 짜증나!! 그리고 조별 포트폴리오 제작할 때 메인 페이지는 사진, 동영상 덕지덕지... 제가 했지만 너무 없어보여! 그 이후로 사진 잘 쓰지 않게 되었습니다. ~~Nyan Baseball은 배경 자체가 사진인데?~~ <-디자인 할 시간이 없었거든? ㅋㅋㅋ
...
$(window).on("load", function () {
 ...
 <div align="center"><b>코드블럭 4</b> 고양이 보호협회 리뉴얼 프로젝트 중 스크롤 애니메이션에 따른 배경 이미지 위치를 css로 조정한 부분</div><br>
  ...
  ~~첫 번째 포트폴리오라 제이쿼리를 사용했습니다. 물론 다시 사용하라 하면 자바스크립트 써야지 ㅋㅋ~~ 코드펜이나 다양한 플러그인 홈페이지에 레퍼런스로 제시된 플러그인은 개발자가 원하는 애니메이션 혹은 인터페이스를 단순한 코드로 구현하도록 하는 일종의 편법(?)입니다. 물론 플러그인을 사용하게 되면 어렵고 복잡한 코딩 과정을 거치지 않아도 되지만, 저는 이러한 플러그인을 쓰지 않습니다. 솔직히 말해서 부작용이 너무 심해, 내 코드에 금 갈까봐 안 써요 ㅠㅠ ㅋㅋㅋ
 <hr>
...
여느 호텔 사이트와 달리 레트로하고 이용하기 힘든 [기존 호텔 사이트](https://fairmont-seoul.com/)에 디자인적인 요소와 인터페이스적인 요소를 한 층 업그레이드 시켜 새로운 사이트로 리뉴얼 하였고 로그인, 정보수정, 회원가입, 고객센터 등과 같은 부수적인 기능들을 추가하였습니다. ~~ios 모바일 환경에서는 메인 페이지의 동영상이 재생되지 않는 오류가 발생한다. 그거 고치려고 지금도 고군분투 중이다.~~
