...
- [Coding Style](#coding-style)
  ...
  - [제이쿼리보다 바닐라 자바스크립트를 선호해요](#제이쿼리보다-바닐라-자바스크립트를-선호해요)
...
> <h3>:school:&nbsp&nbsp&nbsp2022.08 ~ 2023.01</h3> 영등포 방송정보국제교육원 UI/UX기반웹퍼블리싱&프론트엔드 과정 수료<br>
  
### 경력
> <h3>:office:&nbsp&nbsp&nbsp강남 패스트뷰</h3>
> <h4>2021.06 ~ 2021.08</h4>
> <p>디자인팀, 광고팀 인턴</p>
  
> <h3>:office:&nbsp&nbsp&nbsp</h3>
> <h4>2023.04 ~ 재직 중</h4>
> <p>웹 프로그래머</p>
...
<p>
<h3 align="center">"제 손을 거친 이상, 디자인에 있어 무조건 고양이가 빠질 수 없습니다."</h3>
...
<p align="justify">
퀄리티가 높은 사진은 프로젝트의 질을 한층 더 높여줍니다. 하지만 과도한 사진 사용은 포트폴리오의 건강(?)에 상당한 해를 끼칠 수 있기 때문에 필자는 사진 사용을 꺼려합니다. 그리고 사진 쓰면 랜딩 시간이 길어져... 그래서 짜증나!! 그리고 조별 포트폴리오 제작할 때 메인 페이지는 사진, 동영상 덕지덕지... 제가 했지만 너무 없어보여! 그 이후로 사진 잘 쓰지 않게 되었습니다. <s>Nyan Baseball은 배경 자체가 사진인데?</s> <i><-디자인 할 시간이 없었거든? ㅋㅋㅋ</i>
</p>
...
@mixin responsive($break) { // 지원 최소 크기 390px
 ...
 ```
 <div align="center"><b>코드블럭 1.1</b> Nyan Baseball 프로젝트 중 모바일, 태블릿, 데스크탑 반응형 mixin</div><br>
 ...
 ```
 <div align="center"><b>코드블럭 1.2</b> Kidcat Nyantopia 프로젝트 중 svg 게이지 애니메이션</div><br>
 
<p align="justify">
SASS 코딩에서 중단점을 잡을 때도, 자바스크립트 코딩에서 함수의 매개변수를 설정할 때도, 고양이 이름은 빼 놓지 않습니다. 하지만 여러 사람이 만져야 하는 그룹 프로젝트에는 다음과 같은 방법을 쓰다 싸다구를 얻어맏을 수 있겠죠? ㅎㅎ
</p>
...
giveupBtn.addEventListener('click', () => {
 ...
 ```
 <div align="center"><b>코드블럭 3</b> Kidcat 5X5 Game 프로젝트 중 .click(function(){}) 대신 addEventlistener를 사용한 부분</div><br>
 
<p align="justify">
 제이쿼리는 자바스크립트의 기능을 원할하게 사용할 수 있는 오픈 소스 기반 자바스크립트의 라이브러리입니다. 제이쿼리를 사용했을 경우, 바닐라 자바스크립트에 비해 코드의 길이가 짧아지고 시간 또한 절약이 된다는 장점이 있지만 저는 제이쿼리를 꺼려하는 편입니다. 제이쿼리 자체가 자바스크립트에 비해 정확성이 약간 떨어지고 버전마다 지원하지 않는 기능도 있기 떄문에 크로스 브라우징에 있어 치명타를 입는 것 같습니다. 무엇보다 더 치명적인 단점은, 사용할수록 랜딩 시간이 길어지는 개떡같은 경우가 종종 발생한다는 것이지요... 이런 저런 이유로 저는 조별 프로젝트 때 제이쿼리로 크게 데였기 때문에 바닐라 자바스크립트를 만을 활용하여 코딩하고 있습니다.
</p>  
...
$(window).on("load", function () {
 ...
 ```
 <div align="center"><b>코드블럭 4</b> 고양이 보호협회 리뉴얼 프로젝트 중 스크롤 애니메이션에 따른 배경 이미지 위치를 css로 조정한 부분</div><br>
 
 <p align="justify">
  <s>첫 번째 포트폴리오라 제이쿼리를 사용했습니다. 물론 다시 사용하라 하면 자바스크립트 써야지 ㅋㅋ</s> 코드펜이나 다양한 플러그인 홈페이지에 레퍼런스로 제시된 플러그인은 개발자가 원하는 애니메이션 혹은 인터페이스를 단순한 코드로 구현하도록 하는 일종의 편법(?)입니다. 물론 플러그인을 사용하게 되면 어렵고 복잡한 코딩 과정을 거치지 않아도 되지만, 저는 이러한 플러그인을 쓰지 않습니다. 솔직히 말해서 부작용이 너무 심해, 내 코드에 금 갈까봐 안 써요 ㅠㅠ ㅋㅋㅋ 게다가 플러그인은 제 코드를 플러그인에다 맞춰야 한다는 치명적인 단점이 있죠. <s>쌍팔년도 군대처럼 사이즈 상관 없이 주는 대로 입다 결국 옷 사이즈에 내 몸을 맞추게 되는... 그런 원시적인 방법은 싫어요!!</s>
</p>
...
const spec_fireworks = (cat) => {
 ...
 ```
 <div align="center"><b>코드블럭 5.1</b> Kidcat Nyantopia 프로젝트 중 폭죽 효과를 애니메이트로 구현한 부분</div><br>
...
function pj_kitty_ani(neko) {
 ...
 ```
 <div align="center"><b>코드블럭 5.2</b> Kidcat Nyantopia 프로젝트 중 고양이 발 애니메이션을 애니메이트로 구현한 부분</div><br>
 
  <p align="justify">
4차 산업 시대에 들어 인터페이스 혹은 애니메이션이 없는 웹페이지를 찾기 어렵죠... 특히 요즘 나오는 사이트는 모두 애니메이션이 구현된 채로 호스팅 될 정도로 웹 제작에 있어 애니메이션은 필수적으로 작용합니다. 저 또한 프로필 포트폴리오를 제작할 때 50가지 이상의 애니메이션을 부여하였습니다. 덕분에 바닐라 자바스크립트의 메인 코드도 무려 3000줄 가까이 기록한 대기록(?)을 세우게 되었죠 ㅋㅋ
  </p>
...
<table>
 ...
 <p align="justify">
레트로한 디자인과 투박한 정보 위주로 구성이 되어 있던 <a href="https://www.catcare.or.kr/">본래 사이트</a>를 현대의 트렌드에 맞게 리뉴얼하여 인터페이스적인 요소와 비쥬얼적인 요소를 한 층 업그레이드 시켰습니다.
</p>
...
<table>
 ...
 <p align="justify">
  여느 호텔 사이트와 달리 레트로하고 이용하기 힘든 <a href="https://fairmont-seoul.com/">기존 호텔 사이트</a>에 디자인적인 요소와 인터페이스적인 요소를 한 층 업그레이드 시켜 새로운 사이트로 리뉴얼 하였고 로그인, 정보수정, 회원가입, 고객센터 등과 같은 부수적인 기능들을 추가하였습니다. <s>ios 모바일 환경에서는 메인 페이지의 동영상이 재생되지 않는 오류가 발생한다. 그거 고치려고 지금도 고군분투 중이다.</s>
</p>
...
![md_footer](https://user-images.githubusercontent.com/114633489/217474872-f4aa3e25-f0b1-46d5-be9b-4ad4dd4b2706.png)
 ...
 <p align="justify">
  <s>여기까지 긴 글을 읽어 줘서 고맙다냥!</s> 지금까지 저의 글을 읽어 주셔서 대단히 감사드립니다. 사실 저도 개발은 6개월 이전 부터 시작했고, 그 전에는 HTML, CSS, Javascript 존재 조차 몰랐습니다. 농담 같지만 이건 실화입니다... 사실 1년 전까지만 해도 저는 애니메이션을 전공했고, 웹 디자인도 살짝 건드려 본 정도가 전부였으며, 이 마저도 제대로 하지 못해 강점을 어필하지 못했습니다. 늦게 시작한 개발이지만, 흥미를 갖게 되었고, 결국 지금... 프론트 엔드 개발자를 넘어 개발자의 최고 지위인 풀스택 개발자를 노리고 있습니다. 아 제 이야기가 길어졌네요... 중간 중간에 궁금한 점이 있거나 이야기 하고 싶은 것들이 있으면 nonwhiskerscat@gmail.com으로 메일 보내십시오. 친절하게 답변하겠습니다.
</p>
